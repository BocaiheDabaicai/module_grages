...
---
# 基础创建模板
> 本处集成了 vue3-smooth-dnd 实现组件拖拽效果  

该模板集成了信息输入、信息选择、集成根据选择类型显示不同响应组件，包括拖动列表、勾选框、图片上传、文本域描述  
代码集成度稍高，有部分功能模块存在深化和拓展性，比如文件上传功能、拖动列表  
主要内容如下：

<script setup>
import CreateTemplate from "../../../src/pages/system/normal/createTemplate.vue"
</script>
...
文件上传功能已经将自动上传功能关闭。  
如果需要使用，则配置好对应的系统上传路径以及`header`中的`token`属性，才能够成功将文件上传，  
详情请见功能库**文件模块**
